...
if(class_exists('myCRED_Hook')){
	class vortex_like_posts_mycred extends myCRED_Hook {
		...
		function __construct( $hook_prefs, $type ) {
			parent::__construct( array(
				...
				'defaults' => array(
					...
					'dislike'   => array(
						'creds'	 => 0,
						'log'	 => '%plural% deduction for dislike(post)',
						'author' => '-1',
					),
					'canceled'	=> "Vote canceled(post)"
				)
			), $hook_prefs, $type );
		}
		...
		public function run() {
			...
			// that either.
			add_action( 'vortex_post_dislike',  array( $this, 'vortex_post_update' ),10,4 );
		}

		/**
		 * Check if the user qualifies for points
		 */
		public function vortex_post_update($like,$dislike,$id,$post_id) {
			$user_id = $id;
			$post_author_id = get_post_field( 'post_author', $post_id );
			if($user_id != 0 && $post_author_id != $user_id){
	...
	<ol class="inline">
		...
		<li>
			<label for="<?php echo $this->field_id( array( 'like' => 'author' ) ); ?>"><?php _e( 'Content Author', 'mycred' ); ?></label>
	...
	<ol class="inline">
		...
		<li>
			<label for="<?php echo $this->field_id( array( 'dislike' => 'author' ) ); ?>"><?php _e( 'Content Author', 'mycred' ); ?></label>
	...
	add_filter( 'mycred_setup_hooks', 'vortex_post_register_my_custom_hook' );
	function vortex_post_register_my_custom_hook( $installed )
	{
		$installed['vortex_like_posts_mycred'] = array(
			'title'       => __( 'Points for like/dislike posts', 'vortex_system_ld' ),
			'description' => __( 'Settings', 'vortex_system_ld' ),
			'callback'    => array( 'vortex_like_posts_mycred' )
		);
		return $installed;
	}
}